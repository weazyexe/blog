...
Как-то так вышло неожиданно, что у меня появился свой сайт с моими текстами. Хочется рассказать очень много всего: как я до этого докатился, почему блог, как это всё реализовывал, как наполнял своим уже существующим контентом. Начнём по порядку.

## А что случилось

Зайдём немного издалека. В конце марта 2023 года мне пришла в голову идея приложения, которым бы я реально хотел пользоваться – это RSS-читалка. Казалось бы, заходи в Google Play, вбивай "rss reader" и выбирай из огромного количества вариантов. Но я решил, что я не хочу идти по столь простому пути и принял волевое решение написать свою читалку.

У меня было много хотелок по поводу этого приложения, наверное следует написать отдельный текст про приложение, но в списке ниже приведу то, что мне точно хотелось бы иметь в приложении:
- красивый UI;
- прокачаться в техническом и продуктовом плане;
- ну если мне уже пришла навязчивая идея в голову, как я от неё избавлюсь?

Более подробно я опишу историю создания Fonto в отдельном тексте, сейчас это немного оффтоп.

## Telegram

Так вот, вначале я просто писал приложение и всё, никуда и никому про это не говорил, делал в стол. Но потом с каждой новой фичей приложение росло и мне хотелось делиться опытом написания приложения с нуля. Для этого я выбрал ближайшую знакомую себе площадку – Telegram. Я создал свой канал и начал писать туда всё, что связано с этим приложением. Писал так наверное полмесяца, это было похоже на какой-то дневник разработки, ведь из подписчиков был только я сам у себя. Но я подумал, что будет круто, если я напишу хотя бы парочке друзей и скажу, мол, вот, занимаюсь такой-то хернёй, приходите почитать. И они пришли!

Писал какие-то свои мысли, новости в разработке приложения, кидал кучу видосиков с записью экрана приложения, делился прогрессом. Но я понял, что иногда хочется написать немного больше текста и иметь бóльшие возможности для форматирования текста. Telegram даёт кучу возможностей по форматированию (выделение **жирным**, *курсивом*, [оформление ссылок](https://www.youtube.com/watch?v=dQw4w9WgXcQ) и т.д.), но мне нужно было немного больше:

- Хотелось вставить текст, после текста картинку/видео, а потом продолжить текст. Telegram лишь даёт возможность прикреплять к сообщению какое-то медиа. Полезно, но не очень удобно

- Редактор текста в Telegram не очень удобный. Сидишь набираешь текст в маленьком поле набора сообщения, не очень серьёзно

- Большие размеры текста в Telegram читать не очень удобно

И тут я подумал про то, что я хочу писать какие-то длинные тексты, осталось только понять где.
...
+ Готовые и удобные текстовые редакторы с широкими возможностями

Минусы:
- Нужно проходить модерацию на первые публикации
- Нужно писать тексты, подходящие под аудиторию. На vc.ru читателям будет не интересно смотреть контент по играм или личным итогам 2022 года
- Аудитория бывает иногда токсичной без особых на то причин, а мне не нравятся, когда меня кроют матом
- У меня уже есть пара текстов на [habr.ru](https://habr.ru), которые бы хотелось хранить где-то у себя, но они написаны от аккаунта [Surf](https://surf.ru), где я работал на время написания статей. А эти тексты уже были даже немного подредактированы и в конец текста добавили рекламы Telegram каналов от Surf. Вообще без негатива, компания мне помогала писать этот текст – редактура, иллюстрации, публикация – но мои тексты – это мои тексты и мне хотелось бы редактировать их самостоятельно
- Очень важная штука: вставка видео. Habr наотрез отказывается добавлять поддержку AV1, H264 или других кодеков видео, предлагая использовать для анимированного контента формат GIF. Ну или встраивать embed с YouTube. То есть, если я хочу показать 5-секундный ролик с какой-то фичей из своего приложения, мне надо залить ролик на YouTube или конвертировать в GIF. GIF – это устаревший формат. Он весит много, поддерживает всего 256 цветов и обладает таким себе качеством.

### Вастрик.Клуб

Я решил отделить [эту площадку](https://vas3k.club) от остальных, потому что мне очень близки идеи этого сообщества, там очень много разных авторов по разным тематикам и что бы ты не писал, ты найдешь своего читателя. Я думаю, что есть большой смысл там публиковаться и скорее всего с некоторыми текстами я туда залечу. Но мне всё равно хотелось какое-то своё место, где тексты лежат у меня и куда я смогу опубликовать любую дичь и меня, наверное, поймут
...
- [Zola](https://www.getzola.org/) – тоже генератор статических сайтов из Markdown, использующий, как мне показалось, не очень популярный язык шаблонов [Tera](https://tera.netlify.app/). Написан на Rust, следовательно blazingly fast из коробки ⚡️ 🤡 (вообще он правда очень быстрый)

Я выбрал Zola, потому что он мне субъективно понравился, показался удобнее и в целом подходил по моим критериям, кроме написания кода, но об этом дальше.
...
1. **GIF** – самый очевидный вариант, но самый ужасный. Весит много, качество отвратительное, нет поддержки alpha канала.
...
В качестве итога вы можете наблюдать красивый анимированный логотип в шапке на мобильных девайсах и слева для десктопов ✨

### Контент

Дальше нужно было выложить контент, который у меня уже был на данный момент – несколько статей с хабра. И тут начались приколы. Надо было всё скопировать с хабра, обернуть всё в Markdown и нормально вставить картинки. По умолчанию Markdown вставляет картинки в максимальном разрешении и это смотрится отстойно. Чтобы исправить положение, мне на помощь пришли *Shortcodes*.
...
А вы задумывались, откуда Telegram берёт данные из ссылок из сообщений?
